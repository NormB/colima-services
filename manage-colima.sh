...
#######################################
cmd_restart() {
    header "Restarting Colima Services"
...
cmd_shell() {
    ...
    if [[ -z "$service" ]]; then
        error "Service name required. Example: ./manage-colima.sh shell postgres"
    fi
...
#
#   ALWAYS run './manage-colima.sh backup' before reset!
#
#   Requires explicit "yes" confirmation (case-insensitive).
#   After reset, run './manage-colima.sh start' to recreate VM.
#######################################
cmd_reset() {
    ...
    info "Run './manage-colima.sh start' to create a fresh VM"
}
...
#
#   Check seal status first: ./manage-colima.sh vault-status
#
...
cmd_vault_unseal() {
    ...
    if [ ! -f "$vault_keys_file" ]; then
        error "Vault keys file not found: $vault_keys_file. Run './manage-colima.sh vault-init' first"
    fi
...
#
#   If Vault is sealed:
#     - Run './manage-colima.sh vault-unseal' to unseal
...
#     export VAULT_TOKEN=$(./manage-colima.sh vault-token)
...
cmd_vault_token() {
    if [ -f "${HOME}/.config/vault/root-token" ]; then
        ...
        error "Root token file not found. Run './manage-colima.sh vault-init' first"
    fi
}
...
#   Usage examples:
#     # Save to file
#     ./manage-colima.sh vault-ca-cert > vault-ca.pem
#
#     # Install on macOS
#     ./manage-colima.sh vault-ca-cert | sudo security add-trusted-cert \
#       -d -r trustRoot -k /Library/Keychains/System.keychain /dev/stdin
#
#     # Install on Linux
#     ./manage-colima.sh vault-ca-cert | sudo tee \
...
cmd_vault_ca_cert() {
    ...
    if [ ! -f "$ca_file" ]; then
        ...
        echo
        info "Run './manage-colima.sh vault-bootstrap' first to generate CA certificates"
        exit 1
    fi
...
cmd_vault_show_password() {
    ...
    if [ -z "$service" ]; then
        error "Usage: ./manage-colima.sh vault-show-password <service>"
        echo
        info "Available services: postgres, mysql, redis-1, rabbitmq, mongodb"
        exit 1
    fi
    ...
    if ! command -v vault &> /dev/null; then
        password=$(curl -sf -H "X-Vault-Token: $VAULT_TOKEN" \
            "$VAULT_ADDR/v1/secret/data/$service" | jq -r '.data.data.password')
    else
        password=$(vault kv get -field=password "secret/$service" 2>/dev/null)
    fi
...
main() {
    ...
    case "$command" in
        ...
        help|--help|-h)
            ...
            ;;
        *)
            error "Unknown command: $command\n       Run './manage-colima.sh help' for usage"
            ;;
    esac
}
