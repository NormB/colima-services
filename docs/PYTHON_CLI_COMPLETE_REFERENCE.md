...
This document provides complete feature parity documentation between the original bash script `manage-devstack.sh` (now deleted) and the new `manage-devstack` Python CLI wrapper.
...
**Usage:**
```bash
./manage-devstack start [--profile PROFILE]
```
...
**Usage:**
```bash
./manage-devstack stop [--profile PROFILE]
```
...
**Usage:**
```bash
./manage-devstack vault-ca-cert > ca.pem
...
### Key Files
- `manage-devstack` - Wrapper script (automatically uses .venv)
...
1. **Update your shell history/aliases:**
   - Old: `./manage-devstack.sh start`
   - New: `./manage-devstack start`
