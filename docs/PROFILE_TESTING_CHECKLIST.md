...
docker exec dev-redis-1 redis-cli -a "$(./manage-devstack.sh vault-show-password redis-1 | grep Password | awk '{print $2}')" INFO replication | grep role
...
docker exec dev-redis-1 redis-cli -c -a "$(./manage-devstack.sh vault-show-password redis-1 | grep Password | awk '{print $2}')" CLUSTER INFO | grep cluster_state
...
docker exec dev-redis-1 redis-cli -a "$(./manage-devstack.sh vault-show-password redis-1 | grep Password | awk '{print $2}')" INFO cluster | grep cluster_enabled
...
# Test all databases
docker exec dev-postgres pg_isready -U dev_admin
docker exec dev-mysql mysqladmin -u dev_admin -p"$(./manage-devstack.sh vault-show-password mysql | grep Password | awk '{print $2}')" ping
...
./manage-devstack.py start --profile full --profile reference
```
...
./manage-devstack.py start --profile standard
./manage-devstack.py start --profile minimal --no-detach  # Foreground
./manage-devstack.py start --profile full
```
...
./manage-devstack.py logs --tail 50 vault
```
...
```bash
# Test stop variants
./manage-devstack.py stop                    # Stop all
./manage-devstack.py stop --profile reference  # Stop specific profile
```
...
time ./manage-devstack.py start --profile minimal
time ./manage-devstack.py start --profile standard
time ./manage-devstack.py start --profile full
```
...
./manage-devstack.py start --profile standard
time until ./manage-devstack.py health | grep -q "healthy"; do sleep 5; done
```
