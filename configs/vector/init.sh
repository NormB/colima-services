...
# to reading from ${HOME}/.config/vault/root-token file. Exits if neither
...
if [ -n "$VAULT_TOKEN" ]; then
    ...
    VAULT_TOKEN=$(cat "${HOME}/.config/vault/root-token")
    echo -e "${BLUE}[Vector Init]${NC} Loaded VAULT_TOKEN from ${HOME}/.config/vault/root-token"
else
