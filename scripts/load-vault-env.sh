...
#
# Dependencies:
#   - curl (for Vault health check)
#   - python3 (for read-vault-secret.py)
#   - read-vault-secret.py (in same directory)
#
# Exit Codes:
...
#######################################
# Read a secret field from Vault using Python helper script
# Globals:
#   VAULT_TOKEN - Vault authentication token (exported)
#   VAULT_ADDR - Vault server address (exported)
# Arguments:
...
# Notes:
#   - Uses read-vault-secret.py from same directory
#   - Suppresses stderr to avoid leaking error details
#######################################
read_vault_secret() {
    local path="$1"
    local field="$2"

    # Ensure VAULT_TOKEN is exported for Python script
    export VAULT_TOKEN="${VAULT_TOKEN:-}"
    export VAULT_ADDR="${VAULT_ADDR:-http://localhost:8200}"

    # Get script directory to find read-vault-secret.py
    local script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

    # Use Python script to read from Vault
    local value=$(python3 "${script_dir}/read-vault-secret.py" "$path" "$field" 2>/dev/null)
