...
test_profile_startup() {
    ...
    # Start profile
    if ./manage-devstack.py start --profile "$profile" > /tmp/test-profile-startup.log 2>&1; then
...
test_redis_cluster() {
    ...
    # Initialize cluster
    if ./manage-devstack.py redis-cluster-init > /tmp/test-redis-cluster.log 2>&1; then
        # Verify cluster
        local cluster_state
...
test_reference_profile() {
    ...
    if ./manage-devstack.py start --profile standard --profile reference > /tmp/test-reference-startup.log 2>&1; then
...
run_tests() {
    ...
    if [[ ! -f "./manage-devstack.py" ]]; then
        echo "Error: manage-devstack.py not found"
        exit 1
    fi
